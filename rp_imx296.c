			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		fmt->format.width = sensor->fmt.width;
		fmt->format.height = sensor->fmt.height;
		fmt->format.code = sensor->fmt.code;
		fmt->format.field = sensor->fmt.field;
		fmt->format.colorspace = sensor->fmt.colorspace;
		fmt->format.ycbcr_enc = sensor->fmt.ycbcr_enc;
